...
import {
  ExternalLink, Sparkles, RefreshCw, Check, CheckCircle2,
  Clock, Target, TrendingUp, Brain, Calendar, ChevronDown, Trophy,
} from "lucide-react";
import { generateSmartSheet, markProblemSolved, listSolvedSheetProblems } from "@/lib/sheet.functions";
...
export const Route = createFileRoute("/_authenticated/sheet")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Your AI-personalized multi-platform problem sheet for today." },
    ],
  }),
  component: SheetPage,
});
...
function SheetPage() {
  ...
  const solveMut = useMutation({
    mutationFn: (v: { platform: "codeforces" | "codechef" | "leetcode"; key: string; solved: boolean }) =>
            ...
            {(["en", "bn"] as const).map((l) => (
              <button
                ...
                className={cn("px-3 py-1.5 text-xs font-medium rounded-lg transition",
                  lang === l ? "bg-gradient-brand text-white" : "text-muted-foreground hover:text-foreground")}
              >
                {l === "en" ? "EN" : "বাংলা"}
              </button>
            ))}
          ...
          <div className="flex items-center gap-1 glass rounded-xl p-1">
            {([5, 10, 15] as const).map((n) => (
              <button
                key={n}
                onClick={() => setSize(n)}
                className={cn("px-3 py-1.5 text-sm rounded-lg transition",
                  size === n ? "bg-gradient-brand text-white shadow-glow" : "text-muted-foreground hover:text-foreground")}
              >
                {n}
              </button>
            ))}
      ...
      {sheet && grouped ? (
              ...
              <section key={plat} className="space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{list.length} problem{list.length > 1 ? "s" : ""}</p>
                  </div>
                </div>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {list.map((it, i) => (
        ...
        <GlassCard>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Hit <span className="text-primary">Generate sheet</span> — we'll pick problems across Codeforces, CodeChef, and LeetCode based on your rating, weak topics, and yesterday's progress.
          </p>
          {!doneToday ? null : null}
          <Link to="/platforms"><Button variant="outline" className="mt-4 border-border/60">Manage platforms</Button></Link>
        </GlassCard>
      ) : null}
    </div>
  );
        ...
        <div className="mt-2 flex flex-wrap items-baseline gap-3">
          ...
          {sheet.adaptiveShift !== 0 ? (
            <span className={cn("text-xs px-2 py-0.5 rounded-full font-medium",
              sheet.adaptiveShift > 0 ? "bg-success/15 text-success" : "bg-warning/15 text-warning")}>
              <TrendingUp className="size-3 inline mr-1" />
              difficulty {sheet.adaptiveShift > 0 ? "+" : ""}{sheet.adaptiveShift}
            </span>
          ) : null}
        </div>

        <div className="grid sm:grid-cols-3 gap-4 mt-5">
          <Stat icon={<Clock className="size-4" />} label="Estimated time" value={`${hours ? `${hours}h ` : ""}${mins}m`} />
          <Stat icon={<Brain className="size-4" />} label="Focus topics" value={sheet.focusTopics.slice(0, 3).join(", ") || "Balanced"} />
          <Stat icon={<Sparkles className="size-4" />} label="Difficulty mix"
            value={`${sheet.distribution.easy}E · ${sheet.distribution.medium}M · ${sheet.distribution.hard}H`} />
        </div>

        <div className="mt-4 flex flex-wrap gap-2">
          {(Object.entries(sheet.platformBreakdown) as Array<[keyof typeof sheet.platformBreakdown, number]>).map(([p, n]) =>
            n > 0 ? (
              <span key={p} className="text-xs px-2.5 py-1 rounded-lg glass border-border/40 flex items-center gap-1.5">
  ...
  return (
    <div className="glass rounded-2xl p-5">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">~{goal.problemsRemaining} problems · {goal.etaWeeks}w</span>
      </div>
      <div className="mt-2 flex items-baseline gap-2">
        <span className="text-2xl font-display font-semibold">{goal.current}</span>
        <span className="text-muted-foreground">→</span>
        <span className="text-2xl font-display font-semibold text-gradient-brand">{goal.target}</span>
      </div>
        ...
        {plan.map((d, i) => (
          ...
          >
            <div className={cn("text-[10px] uppercase tracking-wider font-semibold",
              i === todayIdx ? "text-primary" : "text-muted-foreground")}>{d.day}</div>
...
function ProblemCard({
  item, index, lang, solved, onToggleSolved,
}: {
  item: Item; index: number; lang: "en" | "bn"; solved: boolean; onToggleSolved: (s: boolean) => void;
}) {
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 text-[10px] uppercase tracking-wider">
          ...
          <span className="text-muted-foreground">·</span>
          <span className="text-muted-foreground inline-flex items-center gap-1"><Clock className="size-3" />{item.estMinutes}m</span>
        </div>
        <button
      ...
      <a href={item.url} target="_blank" rel="noreferrer" className="block mt-2 font-medium leading-tight hover:text-gradient-brand transition">
        {item.title}
      </a>

      <div className="mt-3 flex flex-wrap gap-1.5">
        {item.tags.slice(0, 3).map((t) => (
          <span key={t} className={cn("text-[10px] rounded-md px-1.5 py-0.5",
      ...
      <a href={item.url} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground">

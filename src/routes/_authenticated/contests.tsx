...
function ContestsPage() {
  ...
  const mut = useMutation({
    mutationFn: (vars: { contest_id: number; contest_name: string; start_time: string; bookmark: boolean }) =>
      toggleFn({ data: vars }),
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">All upcoming Codeforces rounds with live countdowns.</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {data?.upcoming.length === 0 && (
            <GlassCard><p className="text-sm text-muted-foreground">No upcoming contests right now.</p></GlassCard>
          )}
          {data?.upcoming.map((c) => (
            <ContestCard
              key={c.id}
...
function ContestCard({
  contest, bookmarked, onToggle,
}: {
  contest: { id: number; name: string; type: string; durationSeconds: number; startTimeSeconds?: number };
  bookmarked: boolean;
    ...
    <GlassCard hover>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="text-[10px] uppercase tracking-wider text-primary font-medium">{contest.type}</div>
          ...
          <div className="mt-3 text-xs text-muted-foreground flex flex-wrap gap-x-4 gap-y-1">
            <span className="inline-flex items-center gap-1"><Calendar className="size-3" />{new Date(start * 1000).toLocaleString()}</span>
            <span className="inline-flex items-center gap-1"><Clock className="size-3" />{fmtDuration(contest.durationSeconds)}</span>
          </div>
        </div>
        <button
          onClick={() => onToggle(!bookmarked)}
          className={`shrink-0 size-9 rounded-lg grid place-items-center ring-1 transition ${
            bookmarked ? "bg-primary/15 ring-primary/40 text-primary" : "bg-white/[0.03] ring-white/[0.08] text-muted-foreground hover:text-foreground"
          }`}
      ...
      <div className="mt-4 grid grid-cols-4 gap-2">
        {[
          ...
          <div key={x.l} className="rounded-lg bg-white/[0.03] border border-white/[0.06] py-2 text-center">
            <div className="text-xl font-display font-semibold tabular-nums">{String(x.v).padStart(2, "0")}</div>
